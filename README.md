...
| 文件 | 说明 |
|------|------|
| [`ClassNote-0.5.0-setup-x64.exe`](https://github.com/whoever1214/ClassNote-Standalone/releases/download/v0.5.0/ClassNote-0.5.0-setup-x64.exe) | Windows x64 安装程序（约 275 MB，**自包含运行时，免装 .NET**） |
...
- **课堂录音**：NAudio 采集 16kHz 单声道 WAV，自动保存到本地。**声音来源可选（v0.5.0）**：麦克风（教室现场）/ 系统声音（设备外放，WASAPI 回环）/ 两者混合，并可指定具体麦克风或播放设备；手动录音与定时录音共用「设置 → 录音设置」里的默认来源与设备。
...
- **LLM 笔记生成**：调用 OpenAI 兼容 API（默认 DeepSeek），把"转写 + 截图文字"整理为 Markdown 笔记。**设置分「API 配置 / 录音设置」两个页签（v0.5.0）**：填好供应商地址与 API Key 后**自动拉取可用模型并以下拉选择**（也支持手输自定义模型名）。
...
- **科目筛选与右键操作（v0.4.3）**：主页「最近记录」顶部可按**科目筛选**（默认「全部课程」，按最近时间排序；全选 / 批量导出 / 删除只作用于筛选出的记录，不会误删被筛掉的）；每条记录的时间戳带**星期几**，**右键单条记录**可直接「查看笔记 / 导出 PDF / 删除」。
...
产物：`dist/ClassNote-0.5.0-setup-x64.exe`。
