...
| 文件 | 说明 |
|------|------|
| [`ClassNote-1.0.0-setup-x64.exe`](https://github.com/whoever1214/ClassNote-Standalone/releases/download/v1.0.0/ClassNote-1.0.0-setup-x64.exe) | Windows x64 安装程序（约 275 MB，**自包含运行时，免装 .NET**） |
...
- **本地 OCR**：调用 Windows 10/11 内置 OCR 引擎（Windows.Media.Ocr），识别截图文字。**录音期间即完成识别**（v0.7），重复截到的同一张幻灯片其 OCR 文本会被去重，减少交给 LLM 的冗余内容。
...
产物：`dist/ClassNote-1.0.0-setup-x64.exe`。
