...
| 动画 | 页面载入淡入 + 上移（0.26s CubicEase）、录音红点呼吸 + 扩散光环、按钮悬停/按下过渡 |

各页面对应关系：

- **MainPage**：顶部品牌栏（logo + 刷新/设置图标按钮）→ 左栏课程选择（选中竖条高亮）+「开始记录」大按钮；右栏最近记录（课程徽章 / 标题 / 时间 / 状态色点胶囊），双击行查看笔记，空列表有占位提示。
...
- `MainViewModelTests.HasPendingSessions_TrueWhenProcessing` 为**过期测试**：产品代码已按
  EV-01 将 `HasPendingSessions` 收敛为仅统计 `recording/processing`（`MainViewModel.cs:44-45`），
  而该用例仍以 `ended` 状态断言"应视为挂起"，与修复后行为矛盾（用例名亦与状态不符）。
  预期修复：用例改用 `processing` 状态，或改为断言 `ended` 不再触发主页轮询。
