...
| 按钮 | `PrimaryButtonStyle`（渐变主按钮）/ `DangerButtonStyle` / `GhostButtonStyle` / `DangerGhostButtonStyle` / `IconButtonStyle`，带悬停高亮与按下微缩动画 |
| 输入控件 | 圆角 TextBox（聚焦出现品牌色光晕）、自定义 ComboBox 模板（圆角 + 下拉浮层圆角阴影） |
| 列表 | 卡片式 ListView/ListBox 行模板（悬停/选中高亮、选中竖条、状态胶囊），细条滚动条 |
| 动画 | 页面载入淡入 + 上移（0.26s CubicEase）、录音红点呼吸 + 扩散光环、按钮悬停/按下过渡、全局滚轮平滑滚动（380ms QuadraticEase，步长按视口 1/3 自适应） |
...
- **动画驱动**：ScrollViewer 的 `VerticalOffset` / `HorizontalOffset` 为只读依赖属性，无法直接动画化，
  经内部 `ScrollViewerOffsetMediator : Animatable` 代理 + `BeginAnimation`（380ms、`QuadraticEase` 缓出）驱动
  `ScrollToVerticalOffset/ScrollToHorizontalOffset`；动画结束即释放（`FillBehavior.Stop`），拖滚动条 / 键盘滚动不受影响；
  通过 `ScrollChanged` 实时同步外部滚动位置，保证动画基准一致。
- **步长自适应**：每次滚动约视口高度的 1/3（项滚动 / 像素滚动按各自单位生效），连续滚动时动画实时重定向，跟手不卡顿。
