...
- 提供两个方法：
  ...
  - `GenerateMindmapAsync`：输出 JSON 层级结构（`{ name, children[] }`）供思维导图。
- 配置来自 `AppSettings`（用户在主页面「设置」中填写 API Key / BaseUrl / 模型名）。
- 支持备用配置故障切换。
...
5. **思维导图**：`_llm.GenerateMindmapAsync`（失败置 null，不阻断）。
6. 保存笔记，更新会话状态为 `completed`。
...
CREATE TABLE notes (
  ...
  title TEXT, content_markdown TEXT, mindmap_data TEXT,
...
| `RecordingViewModel` | 录音控制、进度、处理状态 | `Start/Stop`、`IsLoading`、绑定 `NoteProcessor` |
